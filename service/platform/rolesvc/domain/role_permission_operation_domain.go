...
import (
	...
	po2 "github.com/galaxy-book/polaris-backend/service/platform/rolesvc/po"
...
func UpdateRolePermissionOperation(orgId int64, userId int64, roleId int64, permissionOperation map[int64][]string) errs.SystemErrorInfo {
	insertPo := []interface{}{}
	...
	for i, i2 := range permissionOperation {
		...
		insertPo = append(insertPo, po2.PpmRolRolePermissionOperation{
			Id:             ids.Ids[k].Id,
			OrgId:          orgId,
			RoleId:         roleId,
			PermissionId:   i,
	...
	transErr := mysql.TransX(func(tx sqlbuilder.Tx) error {
		if len(oldPermission) > 0 {
			_, err := mysql.TransUpdateSmartWithCond(tx, consts.TableRolePermissionOperation, db.Cond{
