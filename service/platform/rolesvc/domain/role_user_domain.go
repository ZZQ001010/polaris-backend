...
func UpdateUserOrgRole(role *bo.RoleBo, orgId int64, currentUserId int64, userId int64, projectId *int64) (int64, errs.SystemErrorInfo) {
	lockKey := consts.UpdateUserOrgRoleLock + fmt.Sprintf("%d:%d:%d", role.OrgId, userId, role.ProjectId)
	...
	mysqlErr := mysql.TransX(func(tx sqlbuilder.Tx) error {
		...
		if role.OrgId != 0 && role.LangCode != consts.RoleGroupProMember {
			//增加新角色
			id, idErr := idfacade.ApplyPrimaryIdRelaxed(consts.TableRoleUser)
			if idErr != nil {
				log.Error(idErr)
				return idErr
			}
			insertErr := mysql.TransInsert(tx, &po.PpmRolRoleUser{
				Id:        id,
				OrgId:     role.OrgId,
	...
	//删除缓存
	clearErr := ClearUserRoleList(orgId, userId, 0)
	if clearErr != nil {
		log.Error(clearErr)
		return 0, clearErr
	}
