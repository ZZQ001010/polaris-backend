...
func GetRolePermissionOperationListByPath(orgId int64, roleIds []int64, path string) (*[]bo.RolePermissionOperationBo, errs.SystemErrorInfo) {
	rolePermissionOperationList := &[]bo.RolePermissionOperationBo{}

	roleIds = slice.SliceUniqueInt64(roleIds)
	for _, roleId := range roleIds {
		list, err := GetRolePermissionOperationList(orgId, roleId)
		if err != nil {
			log.Error(err)
			return nil, err
		}
...
func GetRolePermissionOperationList(orgId, roleId int64) (*[]bo.RolePermissionOperationBo, errs.SystemErrorInfo) {
	...
	if rolePermissionOperationListJson != "" {
		...
		_ = copyer.Copy(rolePermissionOperationListPo, rolePermissionOperationListBo)
...
func ClearRolePermissionOperationList(orgId, roleId int64) errs.SystemErrorInfo {
	key, err5 := util.ParseCacheKey(sconsts.CacheRolePermissionOperationList, map[string]interface{}{
		consts.CacheKeyOrgIdConstName:  orgId,
		consts.CacheKeyRoleIdConstName: roleId,
	})
...
func GetUserRoleListByProjectId(orgId, userId, projectId int64) (*[]bo.RoleUserBo, errs.SystemErrorInfo) {
