...
func UserInitByOrg(userId string, corpId string, orgId int64, tx sqlbuilder.Tx) (int64, errs.SystemErrorInfo) {
	...
	//判断用户是否注册过
	err2 := tx.Collection(userOutInfo.TableName()).Find(db.Cond{consts.TcOutUserId: unionId, consts.TcSourceChannel: consts.AppSourceChannelDingTalk}).One(registeredUserOutInfo)
	if err2 != nil {
		registered = false
		log.Info("当前用户没有注册过")
	}

	err2 = tx.Collection(userOutInfo.TableName()).Find(db.Cond{consts.TcOrgId: orgId, consts.TcOutOrgUserId: userId, consts.TcOutUserId: unionId, consts.TcSourceChannel: consts.AppSourceChannelDingTalk}).One(userOutInfo)
	if err2 == nil {
		...
		logger.GetDefaultLogger().Info("user insert")
		...
		if err2 != nil {
			logger.GetDefaultLogger().Error(strs.ObjectToString(err2))
...
func restoreCancellateUser(userOutInfo *po.PpmOrgUserOutInfo, userDetail *sdk.GetUserDetailResp, tx sqlbuilder.Tx, err errs.SystemErrorInfo) (int64, errs.SystemErrorInfo) {
	logger.GetDefaultLogger().Info("user update ")
	//用户之前被注销掉，恢复状态
