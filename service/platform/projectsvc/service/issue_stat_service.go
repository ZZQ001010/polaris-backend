...
func IssueAndProjectCountStat(reqVo projectvo.IssueAndProjectCountStatReqVo) (*vo.IssueAndProjectCountStatResp, errs.SystemErrorInfo) {
	...
	//
	//参与的归档项目数量,参与项目数量
	filingCount, notfilingCount, err := assemblyProjectCount(reqVo.UserId)
	if err != nil {
		log.Error(err)
		return nil, err
	}
...
func assemblyProjectCount(userId int64) (int64, int64, errs.SystemErrorInfo) {

	relationBo, err := domain.GetProjectRelationByTypeAndUserId(userId, []int64{consts.IssueRelationTypeOwner, consts.IssueRelationTypeParticipant, consts.IssueRelationTypeFollower})
	...
	bos, _, err := domain.GetProjectList(userId, conds, nil, 0, 0)
...
func IssueDailyPersonalWorkCompletionStat(reqVo projectvo.IssueDailyPersonalWorkCompletionStatReqVo) (*vo.IssueDailyPersonalWorkCompletionStatResp, errs.SystemErrorInfo) {
	...
	statBos, err := domain.IssueDailyPersonalWorkCompletionStat(orgId, userId, startDate, endDate)
	if err != nil {
		log.Error(err)
		return nil, err
	}
