...
func UpdateProjectDetail(orgId, currentUserId int64, input vo.UpdateProjectDetailReq) (*vo.Void, errs.SystemErrorInfo) {
	...
	authErr := domain.AuthProject(orgId, currentUserId, projectDetail.ProjectId, consts.RoleOperationPathOrgProProConfig, consts.RoleOperationModify)
	if authErr != nil {
		log.Error(authErr)
	...
	if input.Notice != nil {
		asyn.Execute(func() {
			projectInfo, projectInfoErr := domain.GetProjectInfo(projectDetail.ProjectId, orgId)
